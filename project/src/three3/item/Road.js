import * as THREE from '../../../three/build/three.module.js';
import {
    FBXLoader
} from "../../../three/examples/jsm/loaders/FBXLoader.js"
import {
    TGALoader
} from '../../../three/examples/jsm/loaders/TGALoader.js';

import {
    LoadingManager
} from "../../../three/src/loaders/LoadingManager.js"

export const Road = (() => {

    let i = 0;
    class Road {
        constructor(params) {
            let {
                scene,
                objects,
                position,
                rotation_right
            } = params;
            this.scene_ = scene;
            this.objects_ = objects;
            this.position_ = position;
            this.rotation_ = rotation_right;
            this.LoadModel_();
        }

        LoadModel_() {
            const manager = new LoadingManager();
            manager.addHandler(/\.tga$/i, new TGALoader());
            const loader = new FBXLoader(manager);
            loader.setPath('./src/three/fbx/');
            loader.load('road.fbx', (fbx) => {
                fbx.scale.setScalar(1);
                fbx.scale.z = 0.96
                fbx.traverse(c => {
                    c.castShadow = true;
                    c.receiveShadow = true;
                });
                if (this.rotation_) {
                    fbx.rotation.y = -Math.PI / 2;
                } else {
                    fbx.rotation.y = Math.PI / 2;
                }
                this.fbx = fbx;
                fbx.position.copy(this.position_);
                this.scene_.add(fbx);
                var box = new THREE.Box3().setFromObject(fbx);
                var xH = (box.max.x - box.min.x)
                var zH = (box.max.z - box.min.z)
                var yH = (box.max.y - box.min.y)
                console.log(xH, zH, yH);
            });
        }

        Soundloader(sounds) {
            this.fbx.add(sounds)
        }


        /* animation */
        Update(timeElapsed) { // 회전 시키기 `
            // if (this.keys_.space && this.position_.y == 0.0) {
            //     this.velocity_ = 30;
            // }

            // const acceleration = -60 * timeElapsed;

            // this.position_.y += timeElapsed * (this.velocity_ + acceleration * 0.5);
            // this.position_.y = Math.max(this.position_.y, 0.0);

            // this.velocity_ += acceleration;
            // this.velocity_ = Math.max(this.velocity_, -100);

            // if (this.mesh_) {
            //     this.mesh_.position.copy(this.position_);
            // }
        }
    }



    return {
        Road: Road,
    };
})();